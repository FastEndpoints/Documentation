...
- **403 - Forbidden** is added if any claims/roles/permissions are required by the endpoint.

If the defaults are appropriate for your endpoint, you only need to specify any additional accepts/produces metadata using the **Description()** method like below:
...
In order to override the default accepts metadata for a request DTO so that the endpoint can accept any content-type, simply do the following:
```cs
