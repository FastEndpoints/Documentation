...
Even though you can use the vanilla **WebApplicationFactory&lt;T&gt;** together with the convenient [HttpClient extensions](https://api-ref.fast-endpoints.com/api/FastEndpoints.HttpClientExtensions.html#methods) for sending requests to the WAF, the **FastEndpoints.Testing** package comes with an abstract [App Fixture](https://github.com/FastEndpoints/FastEndpoints/blob/main/Src/Testing/TestFixture.cs#L29) and [Test Class Base](https://github.com/FastEndpoints/FastEndpoints/blob/main/Src/Testing/TestClass.cs#L26) to make testing even more convenient by offering:
