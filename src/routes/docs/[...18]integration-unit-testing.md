...
Even though you can use the vanilla **WebApplicationFactory&lt;T&gt;** together with the convenient [HttpClient extensions](https://api-ref.fast-endpoints.com/api/FastEndpoints.HttpClientExtensions.html#methods) for sending requests to the WAF, the **FastEndpoints.Testing** package comes with an abstract [App Fixture](https://github.com/FastEndpoints/FastEndpoints/blob/main/Src/Testing/AppFixture.cs#L33) and [Test Base](https://github.com/FastEndpoints/FastEndpoints/blob/main/Src/Testing/TestBase.cs#L26) to make testing even more convenient by offering:
...
A [collection-fixture](https://xunit.net/docs/shared-context#collection-fixture) enables you to group multiple test-classes into one, and share a common resource with all test-classes of that collection/group. A test-collection results in all test-methods of the collection running serially and never in parallel just so there's no chance of contention between multiple test-methods from multiple test-classes. A test-collection can be thought of as a mega test-class but physically seperated into multiple class files.

In order to use an **AppFixture** with a collection, define it like you'd normally do. It will be instantiated before any of the tests from the collection is run and torn down once all tests from the collection is complete.

```cs
public class AppForCollectionX : AppFixture<Program> {}
```

Next, you need to create a **collection-definition** inheriting from **TestCollection&lt;TAppFixture&gt;**, which is simply a class used to define a test-collection with a unique name/identifier for that particular test-collection.
