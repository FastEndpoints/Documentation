...
This fixture when instantiated by xUnit for a test-class, bootstraps an instance of our main application (SUT) as an in-memory test-server/web-host. That instance
will be re-used by all the test-methods of the class speeding up test execution as constantly booting up and tearing down a WAF/web-host per each test-method would
lead to slower test execution. If a certain set of tests call for a differently configured state (web-host, fake/seed data, httpclients), you should create a separate
fixture for those tests. Further explanation of this can be found [here](https://github.com/FastEndpoints/FastEndpoints/issues/551#fixture).
