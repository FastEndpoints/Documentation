...
## Disabling Sources Per Property

The default behavior with the binding order leads to a lot less attribute noise on DTO models. However, there may be instances where you'd like to explicitly prevent a certain binding source from being used for a particular DTO property, which can be specified like so:
