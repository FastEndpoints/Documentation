...
**Route&lt;T&gt;()** & **Query&lt;T&gt;()** methods are also only able to handle types that have a static TryParse() method and/or valid JSON arrays as mentioned above. If there's no static **TryParse()** method or if parsing fails, an automatic validation failure response is sent to the client.
