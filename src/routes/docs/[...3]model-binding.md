...
Endpoint handlers are supplied with fully populated request DTOs where the property values are automatically bound from the incoming request without any effort from the developer. This behavior can be overridden as explained [here](#customized-request-dto-binding).
...
Would then be auto bound to a complex type with matching property names such as this:
...
When the incoming JSON field name does not match with the request DTO property name, you can use the **[JsonPropertyName]** attribute instruct the serializer on how to match fields.
...
### Serializer Options
#### Custom Converters
Most complex types can be bound as long as the System.Text.Json serializer can handle it. If it's not supported out of the box, please see the [STJ documentation](https://docs.microsoft.com/en-us/dotnet/standard/serialization/system-text-json-converters-how-to?pivots=dotnet-6-0) on how to implement custom converters for your types. Custom converters can be registered in startup as follows:
...
| Field | Value |
| - | - |
| ActorNames | [ "John Wayne" , "Jack Lemon", "Natalie Wood" ]
...
:::admonition type="info"
**Route&lt;T&gt;()** method is only able to handle types that have a static TryParse() method. [See below](#supported-dto-property-types) how to add parsing support for your own types.
:::
...
:::admonition type="info"
**Query&lt;T&gt;()** method is only able to handle types that have a static TryParse() method. [See below](#supported-dto-property-types) how to add parsing support for your own types.
:::
...
### Mismatched Property Names
If the user's **Claim Type** is different from the name of the DTO property being bound to, simply specify the name of the incoming field using the following overload of the attribute like so:
...
The built-in model binding logic can be completely overridden per endpoint/request DTO combo by creating a class that implements **IRequestBinder&lt;TRequest&gt;** and configuring the endpoint to use your custom binder.
