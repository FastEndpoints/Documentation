...
A serializer error would be returned due to the fact that STJ would try to deserialize the **Id** property from the JSON body while there's no value provided for it in the JSON. The simple solution to this is to decorate properties that would not be present in the JSON body with **[JsonIgnore]** as follows:
