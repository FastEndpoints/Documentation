...
Note: When using the Swagger UI, the property naming convention will be applied to route params by default. Therefore, by default you need to use camel case in your endpoints or turn off the default property naming behavior like so:

```cs |title=Program.cs
.SwaggerDocument(c => c.UsePropertyNamingPolicy = false)
```

Otherwise it will literally send {MyString} or {MyBool} in the URL.
