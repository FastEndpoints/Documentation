...
**Step #1** : Install the FastEndpoints source generator package in each project that contains DTOs:
...
**Step #2** : Do an initial build for the generator to run:
...
Nothing else is needed. Deserialization of request DTOs and Serialization of response DTOs will now use source generated logic instead of reflection.
...
Make sure to use the underlying type instead of a nullable type when calling the **ValueParserFor&lt;T&gt;()** method.
