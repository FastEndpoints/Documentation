...
### Step 2: Implement Atomic Claiming In GetNextBatchAsync

The **GetNextBatchAsync** method of your storage provider must atomically find matching records **and** set their **DequeueAfter** to a future time in a single operation. This is what prevents two workers from claiming the same job. The **ExecutionTimeLimit** property on the search parameters can be used to determine a suitable lease duration.
...
Each **UpdateAndGet** call atomically matches a single unclaimed record and sets its **DequeueAfter** in one operation. No two workers can claim the same record. The loop runs at most **Limit** times and stops early when no more eligible records are found.
