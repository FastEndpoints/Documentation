...
### Step 1: Update The Job Storage Record
...
### Step 2: Implement IDistributedJobStorageProvider

In addition to **IJobStorageProvider&lt;TStorageRecord&gt;**, implement the **IDistributedJobStorageProvider&lt;TStorageRecord&gt;** interface on your storage provider class. This interface has two methods:

- **AtomicGetNextBatchAsync** - atomically finds and claims a batch of jobs that are ready for execution. The engine supplies a pre-built **Match** expression via the search parameters that includes all eligibility checks. Providers that support LINQ expression translation (e.g., MongoDB) can use **p.Match** directly as the query filter.
- **HasPendingJobsAsync** - a lightweight existence check to determine if there are any pending jobs (including future scheduled ones) in the queue. This is used by the job queue engine at startup to decide whether to enable periodic polling. The engine supplies a pre-built **Match** expression via the search parameters (matching `QueueID`, `!IsComplete`, and `ExpireOn >= now`, deliberately excluding `ExecuteAfter` so that future jobs are included).

When this interface is detected by the job queue engine, it will use **AtomicGetNextBatchAsync** instead of **GetNextBatchAsync** for fetching jobs. The base **GetNextBatchAsync** method will not be called, so it can simply return an empty collection.

:::admonition type="warning"
The **AtomicGetNextBatchAsync** method must use a database-level atomic operation to find matching records **and** set their **DequeueAfter** to a future time in a single step. A plain "read then update" approach has a race window where two workers can read the same record before either writes. You **MUST** use an atomic operation provided by your database.
:::
...
```cs
sealed class JobStorageProvider
    : IJobStorageProvider<JobRecord>,
      IDistributedJobStorageProvider<JobRecord>
{
    // not called by the engine when IDistributedJobStorageProvider is implemented
    public Task<ICollection<JobRecord>> GetNextBatchAsync(PendingJobSearchParams<JobRecord> p)
        => Task.FromResult<ICollection<JobRecord>>([]]);

    public async Task<ICollection<JobRecord>> AtomicGetNextBatchAsync(PendingJobSearchParams<JobRecord> p)
    {
    ...
    public async Task<bool> HasPendingJobsAsync(
        PendingJobSearchParams<JobRecord> p)
    {
    ...
    PendingJobSearchParams<JobRecord> p)
{
    return await db
        ...
        .Match(p.Match)
        .Limit(1)
        .ExecuteAnyAsync(p.CancellationToken);
}
```
...
### Step 3: Reset DequeueAfter On Execution Failure
