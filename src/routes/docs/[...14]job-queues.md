...
Similarly to the [Command Bus](command-bus), the same **ICommand** and it's companion **ICommandHandler&lt;TCommand&gt;** is used to define your execution logic such as the following:
...
A job encapsulating the command is created and added to a queue for that type of command. There's a job queue per each command type in your application. If there's 10 command types, there'd be 10 independent queues processing jobs.

### Execution Options

At the time of queueing, it's possible to specify a future point of time for the command/job to be executed instead of immediately, which is the default behavior if you don't specify anything. 
...
Job queues are not enabled by default and must be configured at startup. Since job queues are designed to be reliable and not lose any commands in case of server restarts/crashes, it's required for you to implement a storage provider on any database/storage medium of your choice. How to implement the storage provider is discussed below. For now, let's focus on the startup configuration.
...
By default, each queue will process multiple commands in parallel. The default limit is the number logical processors of the machine. For example, if the server has 4 cores/threads, only 4 commands of the same type will ever execute at any given time. You can customize the max concurrency setting at startup like this:
```cs
.UseJobQueues(o => o.MaxConcurrency = 2);
```

Queued jobs can be given a certain amount of time to execute. Command executions exceeding that time limit would automatically get cancelled and retried. By default however, commands are allowed to execute without a limit. You can specify a maximum execution time like like so:
```cs
.UseJobQueues(o => o.ExecutionTimeLimit = TimeSpan.FromSeconds(10));
```

Specifying the limits like above applies to all types of commands, which you can override per type like so:
```cs
.UseJobQueues(o =>
{
...
The storage record entity is simply a POCO encapsulating the actual command DTO. As for the storage provider class, it simply needs to delegate data access to whatever database/storage engine that stores the jobs as shown below:
...
sealed class JobStorageProvider : IJobStorageProvider<JobRecord>
{
    ...
    public Task OnHandlerExecutionFailureAsync(JobRecord job, Exception e, CancellationToken c)
    {
        ...
        // or delete the entity if retry is unnecessary.
        return db
            .Update<JobRecord>()
            .MatchID(job.ID)
    ...
    public Task PurgeStaleJobsAsync(StaleJobSearchParams<JobRecord> p)
    {
        ...
        // do whatever you like with the stale (incomplete or expired) jobs.
