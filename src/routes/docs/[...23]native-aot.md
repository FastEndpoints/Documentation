...
The following information only applies if your app needs to expose swagger documents publicly in the production AOT build. If there's no such need, the configuration can be [greatly simplified](#simplify-swagger-setup).
:::
...
Unlike standard JIT builds, Native AOT applications do not support **WebApplicationFactory** for integration/end-to-end testing. In a JIT environment, you can spin up an in-memory instance and modify its behavior by swapping services in the Dependency Injection container.

For Native AOT, you must instead rely on **Black Box Testing**. This approach treats the application as a sealed executable, providing no control over internal logic beyond passing environment variables. Testing typically involves launching the AOT compiled binary in a separate process and interacting with its public REST API endpoints as a real client would.
...
- Builds your application in AOT mode and launches it as a separate process.
