...
Starting from .NET 7.0, Minimal APIs introduced the ability for endpoints to return multiple results/outcomes packed into a union type called [Results&lt;T1,T2,...&gt;](https://learn.microsoft.com/en-us/aspnet/core/fundamentals/minimal-apis/responses?view=aspnetcore-7.0#resultstresult1-tresultn). The results are generated by calling static methods on the [TypedResults](https://learn.microsoft.com/en-us/aspnet/core/fundamentals/minimal-apis/responses?view=aspnetcore-7.0#typedresults-vs-results) class. 

FastEndpoints also supports this strategy simply by specifying the response DTO type of the endpoint's generic parameter to the desired union type and overriding the **ExecuteAsync(...)** handler method instead of the usual **HandleAsync(...)** method.

```cs
public class MultiResultEndpoint 
    : Endpoint<MyRequest, Results<Ok<MyResponse>, NotFound, ProblemDetails>>
{
    public override void Configure() => Post("multi-result");

    public override async Task<Results<Ok<MyResponse>, NotFound, ProblemDetails>> ExecuteAsync(
        MyRequest req, CancellationToken ct)
    {
        await Task.CompletedTask; //some async work
