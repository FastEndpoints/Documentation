...
- If there's a Validator associated with the endpoint, a **400 - Bad Request** "produces metadata" is added. 
- If the endpoint is not accessible anonymously, a **401 - Unauthorized** is added. 
- If any claims/roles/permissions are required by the endpoint, a **403 - Forbidden** is added.
...
#### Clear only Accepts metadata
In order to clear only the auto added "accepts metadata", you can call the following extension method:
```cs
Description(x => x.ClearDefaultAccepts())
```

#### Clear only Produces metadata
...
Support for JWT Bearer Auth is automatically added. If you need to disable it, simply disable it like so:

```cs |title=Program.cs
builder.Services.SwaggerDocument(o => o.EnableJWTBearerAuth = false);
```
...
```cs
builder.Services.SwaggerDocument(o => o.AutoTagPathSegmentIndex = 2);
```
...
The generated swagger document may contain empty request schemas due to all properties being removed by the operation processor. If you need them removed from the swagger doc, instruct the operation processor to do so by using the following property:
```cs
builder.Services.SwaggerDocument(o => o.RemoveEmptyRequestSchema = true);
```
