...
#### Default scheme with ASP.NET Identity customization
Explicitly setting the default auth scheme as above is essential when [customizing identity models](https://learn.microsoft.com/en-us/aspnet/core/security/authentication/customize-identity-model?view=aspnetcore-7.0) because the **AddIdentity<TUser,TRole>()** call makes **Cookie** auth the default scheme. This may not be the desired result when your application also registers **JWT** auth and you'd expect JWT to be the default. In which case, simply register your auth pipeline as follows:
