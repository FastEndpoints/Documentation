...
This is a globally applicable setting and it's not possible to specify it per swagger document. Also note, if your endpoint class names are not unique, enabling this setting will not be possible unless you manually set a unique name per endpoint as the following.

## Display Operation IDs

You can display operation IDs in the swagger UI by adding the following configuration:

```cs
app.UseSwaggerGen(uiConfig: c => c.AdditionalSettings["displayOperationId"] = true);
```
