...
{
    public Task PreProcessAsync(TRequest req, HttpContext ctx, List<ValidationFailure> failures)
    {
...
```
The generated extension method name will end with the name of your assembly. If your services are in multiple projects, you can add the source generator to each one and separate extension methods will be created per assembly.
