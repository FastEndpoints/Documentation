...
Both packages are currently published as beta packages and are versioned independently of the main FastEndpoints package.

Nothing is exposed by default. An endpoint only becomes visible to agents after you explicitly opt it in.
...
Call **UseMcp()** after **UseFastEndpoints()** so the FastEndpoints endpoint registry is ready.
...
Call **UseA2A()** after **UseFastEndpoints()** so the endpoint registry is ready.
...
If no explicit **Id** is supplied, FastEndpoints uses the endpoint **Summary** text if set, otherwise the endpoint type name. The generated id is converted to **snake_case**, sanitized, and truncated to 64 chars.

Duplicate visible skill ids are rejected. A hidden duplicate does not shadow a visible skill.
