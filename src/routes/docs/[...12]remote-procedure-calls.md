...
<p class="p-4">
    <b>Bottom Line:</b> If you need a reliable event broadcasting system, either use a dedicated message broker or implement a custom storage provider for event persistence as described <a href="#reliable-event-queues-with-persistence">here</a>. If using the default in-memory storage provider, use it only as the means of transport/distribution of messages to the remote nodes.
</p>
:::
...
Then, create an implementation of [IEventPublisherStorageProvider](https://github.com/FastEndpoints/FastEndpoints/blob/main/Src/Messaging.Remote/Server/Events/Storage/IEventPublisherStorageProvider.cs) such as [this example](https://github.com/FastEndpoints/Event-Queues-Demo/blob/main/Server/PublisherStorageProvider.cs) for MongoDB.
