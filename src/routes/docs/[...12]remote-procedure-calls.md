...
Since both publishers & subscribers hold pending events in memory, it is possible to lose events if the processes are killed/restarted while there are pending operations. I.e. there is no auto resumption of broadcasting/handler execution from where it left off.
