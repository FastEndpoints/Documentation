...
RequirePayment(
    ...
    o =>
    {
        ...
        {
            ["plan"] = "premium",
            ["resourceUrl"] = "https://api.example.com/weather/premium"
        };
    });
```
...
If `extra.resourceUrl` is not provided, FastEndpoints automatically adds the current request URL.
...
- Failed facilitator verification -> `402` with `PAYMENT-REQUIRED`

When the facilitator returns a `paymentResponse` during failed verification, FastEndpoints also forwards it in `PAYMENT-RESPONSE`.
...
- If you need custom facilitator behavior, replace **IX402FacilitatorClient** in the DI container.