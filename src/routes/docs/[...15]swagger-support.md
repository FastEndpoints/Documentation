...
There are two methods of generating C# and TypeScript API clients. You can either enable an endpoint which will provide a download of the generated client or you can cause the client to be generated and saved to disk anytime the swagger middleware is triggered.
...
### Save Client To Disk
Doing the following will cause the API client to be generated and saved to disk at the specified location whenever the swagger middleware is triggered. I.e. when either the swagger ui is loaded or `swagger.json` is accessed.
