...
This package only generates the OpenAPI document. If you want an interactive API explorer/visualization, you can plug in something like [Scalar](https://scalar.com/products/api-references/integrations/aspnetcore/integration) or Swagger UI separately.
...
app.MapScalarApiReference(
    o =>
    {
        o.AddDocuments("Release 1.0", "Release 2.0"); //inform scalar of your doc names
...
To define multiple documents, simply call **.OpenApiDocument()** multiple times each with a unique document names.
...
- **ExcludeNonFastEndpoints** for excluding non-FastEndpoints routes from the document.
...
- **ReleaseVersion** for release-version based document generation.
...
- **ShowDeprecatedOps** for including deprecated operations in the document.
...
Give your OpenAPI document a name via the **s.DocumentName** property and pass the same name to the **MapApiClientEndpoint()** method as shown below. Doing so will register an endpoint at the specified route with which the API client can be downloaded as a zip file.
...
var bld = WebApplication.CreateBuilder(args);
 bld.Services
    ...
    .OpenApiDocument(o =>
    {
        o.DocumentSettings = s => s.DocumentName = "v1"; //must match what's being passed below
    });
...
#### Exporting openapi.json Files

It is also possible to export **openapi.json** files to disk using the same strategy as above by using the method call **app.ExportOpenApiJsonAndExitAsync(...)** and the CLI command **dotnet run --exportopenapijson true**.
