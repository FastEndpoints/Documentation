...
At this point you can have 2 OpenAPI documents (release groups) that look like the following:
...
An endpoint marked as above will be visible in all OpenAPI docs up until **MaxEndpointVersion = 4**. It will be excluded from docs starting from **4** and above. As an example, take the following release groups with just two endpoints:
...
The "release group" strategy above automatically includes endpoints in the system that falls under the OpenAPI document's defined **MaxEndpointVersion**. I.e. the document dictates what will be included. The individual endpoints have no say in which OpenAPI document they want to be listed under. With the "release version" strategy, the inverse is possible where the OpenAPI document simply specifies what "release version" it is like so:
...
It's possible to group a bunch of endpoints together into an OpenAPI document by employing an endpoint filter. I.e. only endpoints in your application that matches a condition/predicate will be included in that particular OpenAPI document.
...
VersionSets.CreateApi(">>Inventory<<", v =>
{
    ...
    v.HasApiVersion(new ApiVersion(1.1));
});
```
...
Define OpenAPI documents for each version your application has:

```cs
bld.Services
   .OpenApiDocument(o =>
   {
       o.DocumentName = "v1"; //must match the ApiExplorer group name
       o.Version = "v1";
