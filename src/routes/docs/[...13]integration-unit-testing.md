...
For passing down route parameters you will have to alter the HttpContext by setting them in the `Factory.Create` call like so:
```cs
var ep = Factory.Create<GetUser>(ctx =>
    {
        ctx.Request.RouteValues.Add("id", "1"); // add as many route parameters here as your endpoint needs
    }, 
    // your (mocked) dependencies here
    A.Fake<ILogger<GetUser>>());
```