...
As mentioned [here](openapi-support#describe-endpoints), by default a **400 - Bad Request** **Produces Metadata** is added automatically for endpoints that have validators associated with them. When you are overriding the default error response by specifying your own error response builder func, you should also set the type of the error response DTO which this automatic behavior uses like so:
