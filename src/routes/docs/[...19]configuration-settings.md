...
If you're using the [union type returning endpoint handlers](https://fast-endpoints.com/docs/get-started#union-type-returning-handler) the above will not be effective (due to a technical constraint) and you'll have to resort to using the [options pattern](https://learn.microsoft.com/en-us/aspnet/core/fundamentals/configuration/options?view=aspnetcore-7.0) like so:
