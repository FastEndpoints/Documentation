    ...
    <div class="site-navbar__inner" bind:this={navbarInner}>
        ...
        <nav class="site-navbar__links" aria-label="Main navigation">
            {#each navbar.links as link}
                <a href={link.slug} class:active={active(link)} target={link.slug.startsWith('http') ? '_blank' : undefined}
                   rel={link.slug.startsWith('http') ? 'noreferrer' : undefined}>{link.title}</a>
            {/each}
        </nav>
