...
The **System.Text.Json** source generator support can be easily enabled with either auto generated or manually created serializer contexts:

#### Auto Generate STJ Serializer Contexts

**Step #1** : Install the FastEndpoints source generator package in each project that contains endpoints (as DTOs are discovered through endpoint declarations during source generation):

```shell | copy
dotnet add package FastEndpoints.Generator
```

**Step #2** : Enable context auto generation in project file:
...
**Step #3** : Do an initial build for the generator to run:

```shell | copy
dotnet build
```

**Step #3** : Register the generated context per assembly/project via the generated extension methods:

```csharp
app.UseFastEndpoints(
       c =>
       {
           // customize serializer options first if needed
           c.Serializer.Options.PropertyNamingPolicy = JsonNamingPolicy.SnakeCaseLower;

           // add the generated serializer contexts next
           c.Serializer.Options.AddSerializerContextsFromMyApp();
       });
```

Nothing else is needed. Deserialization of request DTOs and Serialization of response DTOs will now use source generated logic instead of reflection. [See here](native-aot#jsonserializercontext-generation) for more info on the serializer context generator.

#### Manually Create Serializer Contexts
...
::: admonition type=tip
Swagger UI fails to match route parameters when your route uses for example **{MyString}** but your app is configured to use **camelCase** [property naming policy](configuration-settings#specify-json-serializer-options). This can be alleviated by either:

- Using the same casing in route templates as your naming policy, or
- Disabling OpenAPI naming policy application behavior like so:

```cs |title=Program.cs
.OpenApiDocument(c => c.UsePropertyNamingPolicy = false)
```

This only applies to Swagger UI as its param case sensitivity cannot be configured. Regular Http requests work either way.
:::
...
correct OpenAPI document does however require an [attribute](openapi-documents#adding-query-params-to-openapi).
...
simply add a static **TryParse()** method to your type like in the example below or implement the **IParsable\<TSelf\>** interface.
