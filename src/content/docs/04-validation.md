...
By default, FastEndpoints applies the [property naming policy](configuration-settings#specify-json-serializer-options) of the application to all property names/keys of validation errors for consistency. If this behavior is not desired, it can be disabled like so:

```cs 
app.UseFastEndpoints(c => c.Validation.UsePropertyNamingPolicy = false)
``` 
